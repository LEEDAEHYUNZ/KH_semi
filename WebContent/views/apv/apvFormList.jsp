  	...
  	<div>
        ...
        <script>
        	$(function() {
        		$(".list-area>tbody>tr").click(function(){
        			...
        			location.href = "<%= contextPath %>/detail.ap?ano=" + ano;
        		});
        	});
        </script>
        ...
        <% if(loginEmployee != null && loginEmployee.getEmpNo() == 1) { %>
	        <table width="90%">
	            <tr align="right">
	                <td width="50">
	                    <a href="<%= contextPath %>/enrollForm.ap" class="btn btn-secondary btn-sm">글작성</a>
	                </td>
	            </tr>
	       	</table>
       	<% } %>
        ...
        <div align="center" class="paging-area">
        	<!-- 이전 페이지 -->
        	<% if(currentPage != 1) { %>
        		<button onclick="location.href='<%= contextPath %>/list.ap?currentPage=<%= currentPage - 1 %>'">&lt;</button>
        	<% } %>
            ...
            <% for(int i = startPage; i <= endPage; i++) { %>
            	<!-- 버튼 클릭시 이동 -->
            	<% if(i != currentPage) { %>
            		<button onclick="location.href='<%= contextPath %>/list.ap?currentPage=<%= i %>'"><%= i %></button>
            	<% } else { %>
            		<!-- 현재페이지 새로고침 막기 -->
