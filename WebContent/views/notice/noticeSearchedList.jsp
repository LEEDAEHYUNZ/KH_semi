...
<%
	...
	PageInfo searchedPi = (PageInfo)request.getAttribute("searchedPi");
	
	// 페이징바 변수
	int currentPage = searchedPi.getCurrentPage();
	int startPage = searchedPi.getStartPage();
	int endPage = searchedPi.getEndPage();
	int maxPage = searchedPi.getMaxPage();
%>
<!DOCTYPE html>
<html>
<head>
    ...
    <div class="home-content">
  	...
  	<div>
        ...
        <div align="center" class="paging-area">
        	<!-- 이전 페이지 -->
        	<% if(currentPage != 1) { %>
        		<button onclick="location.href='<%= contextPath %>/search.no?currentPage=<%= currentPage - 1 %>'">&lt;</button>
        	<% } %>
            ...
            <% for(int i = startPage; i <= endPage; i++) { %>
            	<!-- 버튼 클릭시 이동 -->
            	<% if(i != currentPage) { %>
            		<button onclick="location.href='<%= contextPath %>/search.no?currentPage=<%= i %>'"><%= i %></button>
            	<% } else { %>
            		<!-- 현재페이지 새로고침 막기 -->
