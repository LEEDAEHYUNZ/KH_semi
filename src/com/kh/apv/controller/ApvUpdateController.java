...
/**
 * Servlet implementation class ApvUpdateController
 */
@WebServlet("/update.ap")
public class ApvUpdateController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ApvUpdateController() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// 4) Service 단으로 토스
			int result = new ApvService().updateApv(a, at);
			
			// 5) 결과에 따른 응답화면 지정
			if(result > 0) {
				request.getSession().setAttribute("alertMsg", "성공적으로 수정되었습니다.");
				response.sendRedirect(request.getContextPath() + "/detail.ap?ano=" + apvNo);
			}
			else {
